...
import (
	...
	//local module
	ep "MiniProject/restoran/bagian/endpoint"
	pb "MiniProject/restoran/bagian/grpc"
	svc "MiniProject/restoran/bagian/server"

	//utility module
	cfg "MiniProject/restoran/util/config"
	run "MiniProject/restoran/util/grpc"
...
func main() {
	...
	var server pb.BagianServiceServer
	{
		...
		//sebelum mulai yang bawah pastikan servicenya selesai
		...
		service := svc.NewBagian(dbReadWriter)
		endpoint := ep.NewBagianEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCBagianServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterBagianServiceServer(grpcServer, server)
